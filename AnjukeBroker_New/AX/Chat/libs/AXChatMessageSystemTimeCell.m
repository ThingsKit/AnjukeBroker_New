...
- (void)configWithData:(NSDictionary *)data
{
    ...
    if ([data[@"messageType"] isEqualToNumber:@(AXMessageTypeSystemTime)]) {
        ...
        self.systemBgView.frame = CGRectMake(160.0f - size.width / 2.0f - 10.0f, 10, size.width + 20.0f, 25);
        ...
        CGRect labRect = self.systemLab.frame;
        self.systemBut.frame = CGRectMake(200 - 5, 27, 90, labRect.size.height);
        self.systemBut.titleLabel.textColor = [UIColor axChatSystemLinkColor:self.isBroker];
        self.systemBut.titleLabel.font = [UIFont axChatSystemFont:self.isBroker];
        [self.systemBut setTitle:@"打开消息提醒" forState:UIControlStateNormal];
        self.systemBgView.frame = CGRectMake( (self.width - 270) / 2, 12, 270, 45);
        self.systemBgView.hidden = NO;
        ...
        self.systemLab.textColor = [UIColor axChatSystemTextColor:self.isBroker];
        CGSize size = [self sizeOfString:self.systemLab.text maxWidth:180.0f withFontSize:self.systemLab.font];
        self.systemLab.frame = CGRectMake(120.0f - size.width / 2.0f, 15, size.width, size.height);
        CGRect labRect = self.systemLab.frame;
        self.systemBut.frame = CGRectMake(labRect.origin.x + labRect.size.width + 10, labRect.origin.y + 1, 80, labRect.size.height);
        self.systemBut.titleLabel.textColor = [UIColor axChatSystemLinkColor:self.isBroker];
        self.systemBut.titleLabel.font = [UIFont axChatSystemFont:self.isBroker];
        [self.systemBut setTitle:@"添加备注" forState:UIControlStateNormal];
        self.systemBgView.frame = CGRectMake( (self.width - 270) / 2 + 10, 10, 250, 28);
        self.systemBgView.hidden = NO;
        ...
        self.systemBgView.hidden = NO;
    } else if ([data[@"messageType"] isEqualToNumber:@(AXMessageTypeVersion)]) {
        self.systemLab.text = data[@"content"];
        self.systemLab.font = [UIFont axChatSystemFont:self.isBroker];
        self.systemLab.textColor = [UIColor axChatSystemTextColor:self.isBroker];
        CGSize size = [self sizeOfString:data[@"content"] maxWidth:240.0f withFontSize:self.systemLab.font];
        self.systemLab.frame = CGRectMake(25 + 15 + 22, 14, size.width, size.height);
        self.systemBgView.frame = CGRectMake( (self.width - 230) / 2, 10, 230, size.height + AXChatMessageSystemTimeCellMarginTop * 2);
        self.systemBgView.hidden = NO;
    }else if ([data[@"messageType"] isEqualToNumber:@(AXMessageTypeSenpropSuccess)])
    {
        self.systemLab.text = data[@"content"];
        self.systemLab.font = [UIFont axChatSystemFont:self.isBroker];
        self.systemLab.textColor = [UIColor axChatSystemTextColor:self.isBroker];
        CGSize size = [self sizeOfString:self.systemLab.text maxWidth:250.0f withFontSize:self.systemLab.font];
        self.systemLab.frame = CGRectMake(25 + 15, 17 + (45-25) / 2, size.width, size.height);
        self.systemBgView.frame = CGRectMake( (self.width - 270) / 2, 12, size.width + 35, size.height + self.systemLab.frame.origin.y);
        self.systemBgView.hidden = NO;

    }else if ([data[@"messageType"] isEqualToNumber:@(AXMessageTypeWillSenprop)])
    {
        self.systemLab.text = data[@"content"];
        self.systemLab.font = [UIFont axChatSystemFont:self.isBroker];
        self.systemLab.textColor = [UIColor axChatSystemTextColor:self.isBroker];
