...
- (void)removeFriendWithMyPhone:(NSString *)phone deleteUid:(NSArray *)deleteUid compeletionBlock:(void (^)(BOOL))deleteFriendBlock
{
    ...
    NSURL *deleteUrl = [NSURL URLWithString:[NSString stringWithFormat:@"http://chatapi.dev.anjuke.com/removeFriends/13333333333"]];
...
#warning todo
    NSDictionary *params = @{@"phone": @"13333333333"};
...
#warning wait for api back data and add to datacenter;
    }
    if (request.tag == AXMessageCenterHttpRequestTypeDeleteFriend) {
    ...
    if (request.tag == AXMessageCenterHttpRequestTypeUploadImage) {
        ...
        if (receiveDic[@"status"] && [receiveDic[@"status"] isEqualToString:@"ok"]) {
            ...
            imageUrl = [NSString stringWithFormat:@"http://pic%@.ajkimg.com/display/%@/%@x%@c.%@",image[@"host"],image[@"id"],image[@"width"],image[@"height"],image[@"format"]];
        }
        
        NSMutableDictionary *params = [NSMutableDictionary dictionary];
        params[@"msg_type"] = self.imageMessage.messageType;
        params[@"phone"] = self.currentPerson.phone;
        params[@"to_uid"] = self.imageMessage.to;
        params[@"uniqid"] = self.imageMessage.identifier;
        params[@"body"] = self.imageMessage.content;
