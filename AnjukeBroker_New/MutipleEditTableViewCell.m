...
@interface MutipleEditTableViewCell () <UIScrollViewDelegate,  UIGestureRecognizerDelegate>

@property (nonatomic, weak) UITableView *containingTableView;

@property (nonatomic, strong) UIPanGestureRecognizer *tableViewPanGestureRecognizer;

@property (nonatomic, assign) MCellState cellState; // The state of the cell within the scroll view, can be left, right or middle
...
- (CGPoint)contentOffsetForCellState:(MCellState)state;
...
@implementation MutipleEditTableViewCell {
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if([keyPath isEqualToString:kTableViewPanState] && object == _tableViewPanGestureRecognizer)
    {
        if(_tableViewPanGestureRecognizer.state == UIGestureRecognizerStateBegan)
        {
            ...
            if(!inCurrentCell && _cellState != mCellStateCenter)
            {
...
{
    if (_cellState == mCellStateCenter)
    {
        if (self.isSelected)
        {
            [self deselectCell];
...
- (void)hideUtilityButtonsAnimated:(BOOL)animated
{
    if (_cellState != mCellStateCenter)
    {
        [self.cellScrollView setContentOffset:[self contentOffsetForCellState:mCellStateCenter] animated:animated];
        
        if ([self.delegate respondsToSelector:@selector(swipeableTableViewCell:scrollingToState:)])
        {
            [self.delegate swipeableTableViewCell:self scrollingToState:mCellStateCenter];
        }
    }
}

- (void)showLeftUtilityButtonsAnimated:(BOOL)animated {
    if (_cellState != mCellStateLeft)
    {
        [self.cellScrollView setContentOffset:[self contentOffsetForCellState:mCellStateLeft] animated:animated];
        
        if ([self.delegate respondsToSelector:@selector(swipeableTableViewCell:scrollingToState:)])
        {
            [self.delegate swipeableTableViewCell:self scrollingToState:mCellStateLeft];
        }
    }
}

- (void)showRightUtilityButtonsAnimated:(BOOL)animated {
    if (_cellState != mCellStateRight)
    {
        [self.cellScrollView setContentOffset:[self contentOffsetForCellState:mCellStateRight] animated:animated];
        
        if ([self.delegate respondsToSelector:@selector(swipeableTableViewCell:scrollingToState:)])
        {
            [self.delegate swipeableTableViewCell:self scrollingToState:mCellStateRight];
        }
    }
}

- (BOOL)isUtilityButtonsHidden {
    return _cellState == mCellStateCenter;
}
...
- (CGPoint)contentOffsetForCellState:(MCellState)state
{
    ...
    switch (state)
    {
        case mCellStateCenter:
            scrollPt.x = [self leftUtilityButtonsWidth];
            break;
            
        case mCellStateRight:
...
- (void)updateCellState
{
    ...
    {
        MCellState cellState = numState.integerValue;
    ...
    if (!self.cellScrollView.isDragging && !self.cellScrollView.isDecelerating)
    {
        ...
        self.longPressGestureRecognizer.enabled = (_cellState == mCellStateCenter);
    }
    else
    {
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
    if (velocity.x >= 0.5f)
    {
        if (_cellState == mCellStateLeft || !self.rightUtilityButtons || self.rightUtilityButtonsWidth == 0.0)
        {
            _cellState = mCellStateCenter;
        }
        else
        {
            _cellState = mCellStateRight;
        }
    }
    else if (velocity.x <= -0.5f)
    {
        if (_cellState == mCellStateRight || !self.leftUtilityButtons || self.leftUtilityButtonsWidth == 0.0)
        {
            _cellState = mCellStateCenter;
        }
        else
        {
            _cellState = mCellStateLeft;
        }
    }
    else
    {
        CGFloat leftThreshold = [self contentOffsetForCellState:mCellStateLeft].x + (self.leftUtilityButtonsWidth / 2);
        CGFloat rightThreshold = [self contentOffsetForCellState:mCellStateRight].x - (self.rightUtilityButtonsWidth / 2);
    ...
    if (_cellState != mCellStateCenter)
    {
        if ([self.delegate respondsToSelector:@selector(swipeableTableViewCellShouldHideUtilityButtonsOnSwipe:)])
        {
            for (MutipleEditTableViewCell *cell in [self.containingTableView visibleCells]) {
                if (cell != self && [cell isKindOfClass:[MutipleEditTableViewCell class]] && [self.delegate swipeableTableViewCellShouldHideUtilityButtonsOnSwipe:cell]) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    if (scrollView.contentOffset.x > [self leftUtilityButtonsWidth])
    {
        if ([self rightUtilityButtonsWidth] > 0)
        {
            ...
            {
                BOOL shouldScroll = [self.delegate swipeableTableViewCell:self canSwipeToState:mCellStateRight];
                if (!shouldScroll)
                {
    ...
    else
    {
        ...
        if ([self leftUtilityButtonsWidth] > 0)
        {
            if (self.delegate && [self.delegate respondsToSelector:@selector(swipeableTableViewCell:canSwipeToState:)])
            {
                BOOL shouldScroll = [self.delegate swipeableTableViewCell:self canSwipeToState:mCellStateLeft];
                if (!shouldScroll)
                {
                    scrollView.contentOffset = CGPointMake([self leftUtilityButtonsWidth], 0);
                }
