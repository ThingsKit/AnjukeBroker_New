...
- (void)keywordsFinish:(RTNetworkResponse *)response
{
    ...
    self.cellTitleArray = response.content[@"data"][@"companyList"];
...
- (void)doBack:(id)sender{
    ...
    if (self.cellTitleArray != nil) {
        if ([self.cellTitleArray safelyObjectAtIndex:self.currentIndex.row]) {
            ...
            if ([self.delegate respondsToSelector:@selector(processCompanyNameWithDic:)]) {
                [self.delegate processCompanyNameWithDic:selectedCellInfo];
            }
        }
    }
}
